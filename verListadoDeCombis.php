...
<body>
     ...
     <?php
        $consulta= "SELECT id_combi, patente, chasis, modelo, nombre_tipo FROM combis NATURAL JOIN tipos_combi " ;#debaja = 0 es falso
        ...
        if ($resultado){
             ...
             <div>
               ...
               //  echo="funcion";
                 $consulta= "SELECT v.origen, v.id_combi, v.fecha_hora_salida FROM viajes v  WHERE (now()<=fecha_hora_salida)" ;
                 $resultado= mysqli_query($link,$consulta) or die ('Consulta fallida: ' .mysqli_error($link));
                 if ($resultado){
                   while (($valores = mysqli_fetch_array($resultado)) ){
