<?php
 ...
 <body>
    <?php 
         ...
         $noTieneCovid="SELECT c.tiene_covid ,t.numero_tarjeta, t.codigo_seguridad, t.fecha_vencimiento FROM usuarios c INNER JOIN tarjetas_clientes tc ON (c.id_usuario=tc.id_cliente) INNER JOIN tarjetas t ON (tc.id_tarjeta= t.id_tarjeta) Where id_usuario='$id'";
         ...
         if($cliente['tiene_covid']!=0) {
             echo "<script > alert('No puede comprar este viaje ya que ha declarado síntomas compatibles con COVID-19 dentro de los 15 días anteriores a la fecha de salida');window.location='home.php'</script>";
         	 ...
         	 if (!isset($_POST['id_viaje'])) {
         	 	 ...
         	 	 $numero_tarjeta=$_SESSION['tarjeta'];
         	 }else{
       ...
       <hr>     
         	 ...
         	 <center>
         	 	 <p> <b>Origen:</b> <?php echo $datosViaje['origen'];?><br>
                     ...
                     <b>Numero de tarjeta: </b><input type="number" name="numero_tarjeta"  maxlength="16" required="" value="<?php echo $numero_tarjeta?>"></input><br><br>
                     <b>Clave de seguridad: </b><input type="password" name="clave"  maxlength="4" required="" value=""></input><br><br>
